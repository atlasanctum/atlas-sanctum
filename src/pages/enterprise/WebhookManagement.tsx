...
const ALL_EVENTS = [
  'payment.succeeded','payment.failed','subscription.created','subscription.updated',
  'subscription.canceled','project.created','project.verified','credit.minted',
  'credit.transferred','governance.proposal.created','governance.vote.cast',
  'sensor.alert','user.created',
];

const MOCK_HOOKS = [
  { id:'wh_1', name:'Production Alerts', url:'https://hooks.example.com/atlas',
    events:['payment.failed','sensor.alert'], isActive:true, createdAt:new Date().toISOString() },
];
...
export default function WebhookManagement() {
  ...
  useEffect(() => {
    fetch('/api/webhooks', { headers:{ Authorization:`Bearer ${token()}` } })
      .then(r => r.ok ? r.json() : null).then(d => d && setHooks(d.data)).catch(()=>{});
  }, []);

  const loadDeliveries = async (id: string) => {
    if (deliveries[id]) return;
    try {
      const r = await fetch(`/api/webhooks/${id}/deliveries`, { headers:{ Authorization:`Bearer ${token()}` } });
      setDeliveries(d => ({ ...d, [id]: r.ok ? (await r.json()).data : MOCK_DELIVERIES }));
    } catch { setDeliveries(d => ({ ...d, [id]: MOCK_DELIVERIES })); }
  };

  const toggleExpand = (id: string) => {
    if (expanded === id) { setExpanded(null); return; }
    setExpanded(id); loadDeliveries(id);
  };

  const handleCreate = async () => {
    if (!form.name || !form.url || !form.events.length) {
      toast({ title:'Validation', description:'Name, URL, and at least one event required.', variant:'destructive' }); return;
    }
    try {
      ...
      if (r.ok) {
        const { data, secret } = await r.json();
        setHooks(h => [data, ...h]);
        toast({ title:'Webhook created', description:`Secret: ${secret} — save this now.` });
      } else throw new Error();
    } catch {
      setHooks(h => [{ id:`wh_${Date.now()}`, ...form, isActive:true, createdAt:new Date().toISOString() }, ...h]);
      toast({ title:'Webhook created (demo)' });
    }
    setForm({ name:'', url:'', events:[] }); setCreating(false);
  };

  const handleDelete = async (id: string) => {
    await fetch(`/api/webhooks/${id}`, { method:'DELETE', headers:{ Authorization:`Bearer ${token()}` } }).catch(()=>{});
  ...
  const handleRotate = async (id: string) => {
    try {
      const r = await fetch(`/api/webhooks/${id}/rotate`, { method:'POST', headers:{ Authorization:`Bearer ${token()}` } });
      if (r.ok) { const { secret } = await r.json(); toast({ title:'Secret rotated', description:`New: ${secret}` }); return; }
    } catch {}
    toast({ title:'Secret rotated (demo)' });
  };

  const handleTest = async (id: string) => {
    await fetch(`/api/webhooks/${id}/test`, { method:'POST', headers:{ Authorization:`Bearer ${token()}` } }).catch(()=>{});
    toast({ title:'Test event dispatched' });
  };

  const copyUrl = (url: string) => {
    navigator.clipboard.writeText(url); setCopied(url); setTimeout(() => setCopied(null), 2000);
  };
  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-4xl">
      <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} className="mb-8 flex items-center justify-between">
        ...
        <Button onClick={() => setCreating(c => !c)}><Plus className="w-4 h-4 mr-2"/>Add Webhook</Button>
      </motion.div>
        ...
        <motion.div initial={{ opacity:0, y:-8 }} animate={{ opacity:1, y:0 }}>
          <Card className="mb-6 border-primary/30">
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <p className="text-sm font-medium mb-2 text-muted-foreground">Events</p>
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <Button variant="ghost" size="sm" onClick={() => handleTest(hook.id)} title="Send test"><Send className="w-4 h-4"/></Button>
                    <Button variant="ghost" size="sm" onClick={() => handleRotate(hook.id)} title="Rotate secret"><RotateCcw className="w-4 h-4"/></Button>
                    <Button variant="ghost" size="sm" onClick={() => handleDelete(hook.id)}><Trash2 className="w-4 h-4 text-destructive"/></Button>
